...
const Control = () => {
  ...
  const visibleDeviceIds = useSelector(
    (state) => state.screen?.visibleDeviceIds || [],
  );
  ...
  const handleScreenshot = async () => {
    ...
    const loadingToast = toast.loading(
      "Capturing HD screenshots... This may take a moment.",
    );

    try {
      // Build device list from visible devices
      const devices = visibleDeviceIds
        .map((id) => {
          const device = allDevices.find((d) => d.id === id);
          return device
            ? { name: device.name, width: device.width, height: device.height }
            : null;
        })
        .filter(Boolean);
      ...
      const totalWidth =
        images.reduce((sum, s) => sum + s.width * scaleFactor, 0) +
        gap * (images.length - 1) +
        padding * 2;
      const maxHeight =
        Math.max(...images.map((s) => s.height * scaleFactor)) + headerHeight;
      ...
      if (error.code === "ERR_NETWORK" || error.code === "ECONNREFUSED") {
        ...
        toast.error(
          `Capture failed: ${error.response?.data?.details || error.message}`,
          { id: loadingToast },
        );
      }
    } finally {
  ...
  const getIcon = (iconName) => {
    ...
    return (
      <IconComponent
        size={14}
        className={theme === "light" ? "text-zinc-500" : "text-zinc-400"}
      />
    );
  };
  ...
  return (
    <div
      className={clsx(
        "flex justify-around px-9",
        theme === "light" ? "bg-zinc-50" : "bg-[#0a0a0a]",
      )}
    >
      <div
        className={clsx(
          ...
          theme === "light"
            ...
            : "bg-[#0a0a0a] text-zinc-300 border-zinc-800/50",
        )}
      >
        {/* Zoom Controls */}
        <div
          className={clsx(
            "flex items-center gap-3 px-2 py-1.5 rounded border shadow-sm transition-colors",
            theme === "light"
              ? "bg-zinc-100 border-zinc-200"
              : "bg-[#18181b] border-zinc-800/50",
          )}
        >
          <button
          ...
          <span
            className={clsx(
              "font-medium text-xs w-10 text-center select-none",
        ...
        >
          ...
          >
            <div
              className={clsx(
                "w-3.5 h-3.5 bg-white rounded-full absolute top-0.75 transition-all duration-200 shadow-sm",
                syncScroll ? "left-4.5" : "left-1",
              )}
            ></div>
          </button>
        </div>
        ...
        <button
          ...
          className={clsx(
            ...
            theme === "light"
              ...
              : "bg-[#18181b] hover:bg-[#27272a] border-zinc-800/50",
          )}
        >
          <RefreshCw
            size={14}
        ...
        <div
          className={clsx(
            "flex items-center gap-1 p-1 rounded-xl border shadow-sm transition-colors",
            theme === "light"
              ? "bg-zinc-100 border-zinc-200"
              : "bg-[#18181b] border-zinc-800/50",
          )}
        ...
        <button
          ...
          className={clsx(
            ...
            theme === "light"
              ...
              : "bg-[#18181b] hover:bg-[#27272a] border-zinc-800/50 text-zinc-400 hover:text-zinc-200",
            isScreenshotLoading && "opacity-50 cursor-not-allowed",
          )}
        >
          <Camera
            size={14}
            className={clsx(isScreenshotLoading && "animate-pulse")}
          />
          <span className="text-xs font-medium hidden sm:inline">
        ...
        <div
          className={clsx(
            "flex items-center gap-3 p-1 rounded-xl border shadow-sm transition-colors",
            theme === "light"
              ? "bg-zinc-100 border-zinc-200"
              : "bg-[#18181b] border-zinc-800/50",
          )}
        >
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={clsx(
                "flex items-center gap-2 px-4 py-1.5 rounded-lg font-medium transition-colors shadow-sm",
                theme === "light"
              ...
              <form onSubmit={handleSaveProject} className="space-y-4">
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className={clsx(
                      ...
                      theme === "light"
                        ? "bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-blue-500/50"
                        : "bg-[#0a0a0a] border-zinc-800 text-zinc-200 focus:border-blue-500/50",
                    )}
                  />
                </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    >
                      {visibleDeviceIds.length} Selected
                    </span>
                  </div>
                </div>
